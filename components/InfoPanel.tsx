...
function InfoPanelComponent({ account, currentPrice, profit, tradingPair, backtestData, tradingMode }: InfoPanelProps): React.ReactElement {
  const [baseCurrency, quoteCurrency] = tradingPair ? tradingPair.split('-') : ['?', '?'];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center text-sm">
      <div>
        <p>{quoteCurrency} Kontostand: <span className="font-mono text-white">{(account?.quote || 0).toFixed(2)} {quoteCurrency} {modeLabel}</span></p>
        <p>{baseCurrency} Kontostand: <span className="font-mono text-white">{(account?.base || 0).toFixed(6)} {baseCurrency} {modeLabel}</span></p>
        <p className={`font-bold ${profitClass}`}>Gesamtgewinn: {(profit || 0).toFixed(2)} {quoteCurrency} {modeLabel}</p>
      </div>
      <div className="text-left md:text-right">
        {modeDisplay}
        <p className="text-slate-400">{baseCurrency} MiddleMarket Preis: <span className="font-mono text-white text-lg">{(currentPrice || 0).toFixed(2)} {quoteCurrency}</span></p>
      </div>
    </div>
  );
}
